...
function closeArtifactReader() { document.querySelector(".research-outline-reader[open]")?.close(); }
function openArtifactReader(artifact) {
  let dialog = document.querySelector(".research-outline-reader[open]");
  if (dialog) {
    dialog.querySelector("h2").textContent = `${artifact.title} · V${artifact.version || 1}`;
    renderArtifactDetail(artifact, dialog.querySelector(".research-outline-reader-content"));
    return;
  }
  dialog = node("dialog", "research-outline-reader", "");
  dialog.setAttribute("aria-labelledby", "researchOutlineReaderTitle");
  const header = node("header", "research-outline-reader-head", "");
  const title = node("h2", "", `${artifact.title} · V${artifact.version || 1}`);
  ...
  renderArtifactDetail(artifact, content);
  dialog.append(header, content);
  dialog.addEventListener("close", () => { dialog.remove(); if (!document.querySelector(".research-outline-reader[open]")) document.body.classList.remove("research-outline-reader-open"); });
  dialog.addEventListener("click", event => { const rect = dialog.getBoundingClientRect(); if (event.target === dialog && (event.clientX < rect.left || event.clientX > rect.right || event.clientY < rect.top || event.clientY > rect.bottom)) dialog.close(); });
  document.body.appendChild(dialog);
  document.body.classList.add("research-outline-reader-open");
  dialog.showModal();
}
function renderArtifactDetail(artifact, container = null) {
  const detail = container || $("#researchArtifactDetail"); detail.replaceChildren(); detail.hidden = !artifact;
  if (!container && artifact && ["report_outline", "ppt_script"].includes(artifact.type)) { detail.hidden = true; openArtifactReader(artifact); return; }
  if (!artifact) { if (!container) closeArtifactReader(); return; }
  ...
  if (artifact.parent_artifact_id) { const compare = node("button", "ghost-btn", "与父版本对比"); compare.type = "button"; compare.addEventListener("click", () => compareArtifact(artifact, detail)); actions.appendChild(compare); }
  if (!container) detail.appendChild(head); detail.appendChild(actions); const outline = parsedOutline(artifact); const pptScript = parsedPptScript(artifact); const qualitativePpt = parsedQualitativePpt(artifact); if (outline) renderReportOutline(detail, artifact, outline); else if (pptScript) renderPptScript(detail, artifact, pptScript); else if (qualitativePpt) renderQualitativePpt(detail, artifact, qualitativePpt); else detail.appendChild(node("pre", "", artifact.content || ""));
...
function selectArtifact(artifact) { closeArtifactReader(); state.selectedArtifactId = artifact.id; state.targetPptScriptPageId = null; state.taskType = artifact.type === "ppt_script" ? "ppt_script" : "artifact_revision"; if (["research_plan", "analysis", "qualitative_analysis", "report_outline", "ppt_script"].includes(artifact.type)) { state.activeWorkflow = { ...(state.activeWorkflow || {}), task_type: artifact.type === "analysis" ? "data_analysis" : artifact.type === "qualitative_analysis" ? "qualitative_analysis" : artifact.type === "report_outline" ? "report_storyline" : artifact.type === "ppt_script" ? "ppt_script" : "research_plan", parent_artifact_id: artifact.id, artifact_id: null, status: "pending", stages: [] }; renderWorkflow(state.activeWorkflow); } if (["report_outline", "ppt_script"].includes(artifact.type)) { state.selectedFileIds.clear(); state.autoRetrieve = false; } $("#researchChatInput").value = artifact.type === "report_outline" ? "请基于这份报告大纲调整结构：" : artifact.type === "ppt_script" ? "请基于这份 PPT Script 继续修改：" : "请基于这份成果继续修改："; setMobileTab("chat"); renderContextChips(); renderArtifacts(); $("#researchChatInput").focus(); }
