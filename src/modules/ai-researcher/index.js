...
function renderArtifactDetail(artifact) { const detail = $("#researchArtifactDetail"); detail.replaceChildren(); detail.hidden = !artifact; if (!artifact) return; const head = node("div", "research-artifact-detail-head", ""); head.append(node("strong", "", artifact.title), node("span", "", `${artifactLabel(artifact)} · V${artifact.version || 1}`)); const close = node("button", "ghost-btn", "关闭"); close.type = "button"; close.addEventListener("click", () => renderArtifactDetail(null)); head.appendChild(close); const actions = node("div", "research-artifact-detail-actions", ""); const derive = node("button", "secondary-btn", "基于此版本派生"); derive.type = "button"; derive.addEventListener("click", () => selectArtifact(artifact)); const exportBtn = node("button", "ghost-btn", "导出 Word"); exportBtn.type = "button"; exportBtn.addEventListener("click", () => exportWord(artifact.content || "", artifact.title || "AI研究成果")); actions.append(derive, exportBtn); if (artifact.parent_artifact_id) { const compare = node("button", "ghost-btn", "与父版本对比"); compare.type = "button"; compare.addEventListener("click", () => compareArtifact(artifact)); actions.appendChild(compare); } detail.append(head, actions, node("pre", "", artifact.content || "")); }
function renderArtifacts() { const list = $("#researchArtifactList"); $("#researchArtifactCount").textContent = `${state.artifacts.length} 项`; list.replaceChildren(); if (!state.artifacts.length) { list.appendChild(node("p", "research-empty-copy", "AI 回复可保存为调研方案、问卷或访谈大纲。")); renderContextChips(); return; } state.artifacts.forEach((artifact) => { const card = node("article", `research-artifact-card${state.selectedArtifactId === artifact.id ? " selected" : ""}`, ""); const meta = node("div", "research-artifact-meta", ""); meta.append(node("span", "", artifactLabel(artifact)), node("strong", "", `V${artifact.version || 1}`)); card.append(meta, node("h5", "", artifact.title || artifactLabel(artifact))); if (artifact.parent_artifact_id) card.append(node("small", "research-version-parent", "基于上一版本生成")); const actions = node("div", "research-artifact-actions", ""); [["查看", () => renderArtifactDetail(artifact)], ["基于此版本派生", () => selectArtifact(artifact)], ["重命名", () => renameArtifact(artifact)], ["删除", () => deleteArtifact(artifact)]].forEach(([label, action], index) => { const button = node("button", index === 1 ? "secondary-btn" : "ghost-btn", label); button.type = "button"; button.addEventListener("click", action); actions.appendChild(button); }); card.appendChild(actions); list.appendChild(card); }); renderContextChips(); }
...
async function loadArtifacts() { const result = await api(`/projects/${encodeURIComponent(state.project.id)}/artifacts`); state.artifacts = result.artifacts || []; if (state.selectedArtifactId && !state.artifacts.some((artifact) => artifact.id === state.selectedArtifactId)) state.selectedArtifactId = null; renderArtifacts(); }
...
function selectArtifact(artifact) { state.selectedArtifactId = artifact.id; state.taskType = "artifact_revision"; $("#researchChatInput").value = "请基于这份成果继续修改："; setMobileTab("chat"); renderContextChips(); renderArtifacts(); $("#researchChatInput").focus(); }
function clearArtifactContext() { state.selectedArtifactId = null; if (state.taskType === "artifact_revision") state.taskType = "free_chat"; renderContextChips(); renderArtifacts(); }
