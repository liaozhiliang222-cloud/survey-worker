...
5. Configure `HARNESS_BASE_URL`, `HARNESS_USERNAME`, `HARNESS_PASSWORD`, `HARNESS_API_STYLE=dsh-rpc`, `HARNESS_AGENT_PRESET=survey-research`, `HARNESS_MODEL_PROVIDER`, `HARNESS_MODEL`, `HARNESS_TIMEOUT`, `HARNESS_LONG_TASK_TIMEOUT`, and `HARNESS_MAX_CONTINUATIONS` as server-side variables/secrets. Keep `HARNESS_PASSWORD` in the Cloudflare secret store.
