...
1. Create a D1 database and apply `migrations/0001_ai_researcher.sql`, `migrations/0002_research_files_context.sql`, and `migrations/0003_research_memory.sql` in order.
...
The deployed DeepSeek Harness uses its RPC envelope over `POST /api/session.*`, protected by nginx Basic Auth. SurveyKit creates a session with `session.create`, selects the `survey-research` preset and the configured model through `session.selectModel`, renames the session, submits text with `session.prompt`, then polls the lightweight `session.list` running state and reads only the latest message window from `session.history`. While a turn is still running, history inspection is throttled to once every 10 seconds so long streamed answers do not repeatedly transfer thousands of chunk events through the edge runtime. Model selection is also enforced before the first send in a process so existing SurveyKit sessions adopt a new configured default. Structured deliverable tasks use `HARNESS_LONG_TASK_TIMEOUT` and direct-reply mode: prompts prohibit tools, and any observed tool call is cancelled and retried once as a response-only turn. Any request that reaches its deadline is cancelled through `session.cancel` before SurveyKit returns a timeout. A missing or expired session is recreated once and receives the authoritative project context again. The earlier OpenCode REST mode remains available through `HARNESS_API_STYLE=opencode` for compatibility.
