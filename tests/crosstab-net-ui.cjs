...
await p.evaluate(()=>{delete netGroupConfig.C2;autoNetSpec.questions.C2.groups[0].codes.push('999');refreshAutoNetBindings();});assert.match(await p.locator('#autoNetWarnings').innerText(),/999/);assert.equal(await p.evaluate(()=>autoNetBindings.C2.length),8);
console.log('PASS real questionnaire/SAV NET, toggle, edit selection, manual precedence, missing-code warning and actual download');
