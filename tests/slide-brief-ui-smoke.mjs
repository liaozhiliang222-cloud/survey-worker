...
assert.match(app, /data-pptx-report-quality/);
assert.match(app, /ai_report_quality/);
assert.match(app, /auditSlideBriefQuality/);
...
assert.match(stagedNarrativeFlow, /requestAssignmentBatch\(pendingPages\.slice\(0, midpoint\)/);
assert.match(stagedNarrativeFlow, /assignedPages\.size !== compactPages\.length/);
assert.match(stagedNarrativeFlow, /已保留完成结果；再次生成将只续跑缺失页面/);
assert.doesNotMatch(stagedNarrativeFlow, /启用本地兜底补全/);
assert.match(stagedNarrativeFlow, /updateAiReportStageSnapshot/);
assert.match(stagedNarrativeFlow, /auditReportNarrative/);
assert.match(stagedNarrativeFlow, /taskTier: "storyline"/);
assert.match(stagedNarrativeFlow, /stream: false/);
assert.match(app, /function getAiProxyUrl/);
