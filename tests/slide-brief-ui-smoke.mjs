...
assert.match(app, /validation_error:.*original_input:.*invalid_output:/s);
const narrativeFlowStart = app.indexOf("async function generatePptxAiReport()");
const narrativeFlowEnd = app.indexOf("function normalizePptxDimensions", narrativeFlowStart);
const narrativeFlow = app.slice(narrativeFlowStart, narrativeFlowEnd);
assert.match(narrativeFlow, /maxTokens: 3000/);
assert.match(narrativeFlow, /stream: true/);
assert.match(narrativeFlow, /updateNarrativeStreamProgress/);
assert.ok(narrativeFlow.includes("payload = aiPlanner.parseJsonObject(output);"));
assert.ok(narrativeFlow.includes('invalid_output: payload || String(output || "").slice(0, 12000)'));
assert.ok(narrativeFlow.includes("validateReportNarrative(repairedPayload, narrativeContext)"));
