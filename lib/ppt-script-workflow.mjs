...
import { normalizeLayoutBinding } from "./ppt-template-state.mjs";
...
function normalizePage(page, index, evidenceMap, issues) {
  ...
  const normalized = {
    ...
    layout_binding: normalizeLayoutBinding(page.layout_binding),
...
export function normalizePptScript(raw, { projectTitle = "研究项目", outlineArtifact, evidence = [], existingScript = null, targetPageId = "" } = {}) {
  ...
  if (targetPageId && existingScript) {
    ...
    pageDrafts = array(existingScript.pages).map((page) => page.id === targetPageId ? { ...page, ...replacement,
      ...(page.layout_binding?.locked ? { layout_variant: page.layout_variant, layout_binding: page.layout_binding, layout_spec: { ...object(replacement.layout_spec || page.layout_spec), variant: page.layout_variant } } : {}),
      id: page.id, page_number: page.page_number, updated_at: new Date().toISOString() } : page);
  ...
  if (targetPageId && existingScript?.style_profile) Object.assign(style, existingScript.style_profile);
