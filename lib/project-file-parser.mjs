const SUPPORTED_EXTENSIONS = new Set(["docx", "pdf", "txt", "md", "xlsx", "pptx"]);
...
export function validateProjectFile({ fileName, mimeType, size, maxBytes = 10 * 1024 * 1024 }) {
  ...
  if (!SUPPORTED_EXTENSIONS.has(extension)) return { ok: false, code: "FILE_TYPE_UNSUPPORTED", message: "仅支持 DOCX、PDF、TXT、MD、XLSX 和 PPTX。" };
  ...
  const allowedTextAlias = extension === "md" && normalizedMime === "text/plain";
  if (!GENERIC_MIMES.has(normalizedMime) && normalizedMime !== expected && !allowedTextAlias) {
...
export async function parseProjectFile(arrayBuffer, options = {}) {
  ...
  if (["txt", "md"].includes(extension)) {
    ...
    structuredData = { kind: "xlsx_workbook", sheet_count: sheets.length, sheets };
