...
export async function requestDataExecutor(env, payload) {
  ...
  const response = await fetch(url, { method:'POST', redirect:'manual', headers:{'Content-Type':'application/json',Authorization:`Bearer ${env.DATA_EXECUTOR_SECRET}`}, body:JSON.stringify(payload), signal:AbortSignal.timeout(110000) });
  if (response.status >= 300 && response.status < 400) { await response.body?.cancel(); throw Object.assign(new Error('数据服务地址发生重定向，已拒绝转发凭据。'),{code:'DATA_EXECUTOR_REDIRECT'}); }
