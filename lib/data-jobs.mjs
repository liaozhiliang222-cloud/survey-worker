...
export async function executeDataJob({ store, fileStorage, job, leaseDuration = leaseMs, timeout = maxRunMs, heartbeatInterval = 1000, runTool = ({ store, fileStorage, request }) => createDataToolExecutor({ store, fileStorage })(request) }) {
  ...
  const heartbeat = setInterval(() => { store.heartbeatDataJob(job.project_id, job.id, token, leaseDuration).then((live) => { if (!live) controller.abort(new Error('DATA_JOB_LEASE_LOST')); }).catch(() => {}); }, Math.max(10, Math.min(heartbeatInterval, leaseDuration / 3)));
  heartbeat.unref?.();
  try {
