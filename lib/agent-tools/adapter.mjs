...
export async function executeAgentTool({ agentToolId, args, harnessSessionId, callId, requestId, store, fileStorage = null, dataJobRunner, dataJobExecutor, logger = console }) {
  const tool = AGENT_TOOL_REGISTRY[agentToolId];
  ...
  try {
    if (["data_profile", "data_clean", "data_weight", "crosstab"].includes(agentToolId)) {
      ...
      await (dataJobExecutor ? dataJobExecutor(job) : executeDataJob({ store, fileStorage, job, runTool: dataJobRunner }));
