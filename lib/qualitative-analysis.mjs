...
export async function transcriptSearch({ store, projectId, query, filters = {}, transcriptIds = [], limit = 12 }) {
  ...
  // Some models place the scope alongside metadata filters. Keep both scopes
  // restrictive when supplied together; never interpret IDs as respondent data.
  if (!filters || typeof filters !== "object" || Array.isArray(filters) || !Array.isArray(transcriptIds) || (filters.transcript_ids != null && !Array.isArray(filters.transcript_ids))) throw new ToolInputError("访谈筛选参数无效，transcript_ids 必须为数组。", "TRANSCRIPT_FILTER_INVALID");
  const nestedIds = new Set((filters.transcript_ids || []).map(text).filter(Boolean));
  filters = { ...filters };
  delete filters.transcript_ids;
  const allowedIds = new Set((transcriptIds || []).map(text).filter(Boolean));
  const transcripts = (await store.listTranscripts(projectId)).filter((item) => item.status === "ready" && (!allowedIds.size || allowedIds.has(item.id)) && (!nestedIds.size || nestedIds.has(item.id)) && metadataMatches(item, filters));
...
export async function finalizeQualitativeAnalysis({ store, projectId, projectTitle, reply, parentArtifactId = null, transcripts = [], failedFiles = [] }) {
  ...
  if (!citations.length) throw new ToolInputError("本次分析未获得可核验的访谈原声，尚未生成正式定性成果。请检查访谈范围与研究问题后重试。", "QUALITATIVE_EVIDENCE_REQUIRED");
