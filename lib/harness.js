...
function createHarnessAdapter({ env = process.env, fetchImpl = globalThis.fetch, logger = console } = {}) {
  ...
  async function waitForDshTurnEnd(sessionId, baseline, requestId, deadline) {
    ...
    while (Date.now() < deadline) {
      const latest = await rpc("session.history", { sessionId: String(sessionId), maxMessages: 10 }, requestId, Math.max(1, deadline - Date.now()));
  ...
  async function runDshTurn({ sessionId, prompt, requestId, deadline, forbidTools }) {
    const history = await rpc("session.history", { sessionId: String(sessionId), maxMessages: 10 }, requestId, Math.max(1, deadline - Date.now()));
    const baseline = Math.max(-1, ...(Array.isArray(history?.events) ? history.events.map((entry) => Number(entry?.event?.seq ?? -1)) : [-1]));
