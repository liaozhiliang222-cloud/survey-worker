"use strict";
const { readOpenAiStream } = require("./harness-stream.js");
...
function createHarnessAdapter({ env = process.env, fetchImpl = globalThis.fetch, logger = console, executeTool = null } = {}) {
  ...
  async function request(path, body, requestId, maximumMs = timeoutMs(env.HARNESS_TIMEOUT)) {
    ...
    try {
      ...
      if (body.stream && response.headers.get("content-type")?.includes("text/event-stream")) return await readOpenAiStream(response);
  ...
  async function sendOpenAiChat(options) {
    ...
    while (Date.now() < deadline) {
      const body = { model, messages, stream: Boolean(options.streamResponse && options.forbidTools) };
